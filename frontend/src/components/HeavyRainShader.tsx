...
float mapCar(in vec3 p0) { 
    ...
    p.xz = abs(p.xz) - vec2(0.53, 0.96); p.x = abs(p.x);
    r = length(p.yz);
...
float map(const in vec3 p) {
    ...
    if(lh > 0.5) { dL = opU(dL, sdBox(vec3(pdm.x - 12.5, pdm.y - 2.75 - lh, mod(pd.z, 13.) - 6.5), vec3(0.1, 0.25, 3.2))); }
...
float intersect(in vec3 ro, in vec3 rd) {
    ...
    for(int i = 0; i < MARCHSTEPS; i++) {
        h = map(ro + rd * t);
        if(dL < mld) { mld = dL; lint1.xyz = ro + rd * t; lint1.w = abs(dL); }
        if(h < precis) { int1.xyz = ro + rd * t; break; } 
        t += max(h, precis * 2.);
    }
    if(int1.z < -400. || t > 300.) {
        float d = -(ro.y + 0.1) / rd.y;
        if(d > 0.) { int1.xyz = ro + rd * d; } else { return -1.; }
    }
    ro = ro + rd * t; nor1 = calcNormal(ro);
    ro += 0.01 * nor1; rd = reflect(rd, nor1);
    t = 0.0; h = precis * 2.0; mld = 100.;
    for(int i = 0; i < MARCHSTEPSREFLECTION; i++) {
        h = map(ro + rd * t);
        if(dL < mld) { mld = dL; lint2.xyz = ro + rd * t; lint2.w = abs(dL); }
        if(h < precis) { int2.xyz = ro + rd * t; return 1.; }   
        t += max(h, precis * 2.);
    }
    return 0.;
}
...
vec3 shade(in vec3 ro, in vec3 pos, in vec3 nor) {
    ...
    if(pos.y < 0.01) {
        if(abs(pos.x) < 0.1 || abs(abs(pos.x) - 7.4) < 0.1) col = vec3(0.9);
    }    
    ...
    if(abs(pos.x) > 12.9 && pos.y > 9.) {
        ...
        if(ha > 0.95) col = ((ha - 0.95) * 10.) * vec3(1., 0.7, 0.4);
    }
...
vec3 getLightColor(in vec3 pos) {
    ...
    vec3 pd = pos; pd.x = abs(pd.x); pd.z *= -sign(pos.x);
    float ch = hash(floor((pd.z + 18. * time) / 40.));
    vec3 pdc = vec3(pd.x, pd.y, mod(pd.z + 18. * time, 40.) - 20.);
    if(ch > 0.75) {
        pdc.x += (ch - 0.75) * 4.;
        if(sdSphere(vec3(abs(pdc.x - 5.) - 1.05, pdc.y - 0.55, pdc.z), 0.25) < 2.) lcol = vec3(1., 0.05, 0.01);
    }
    if(pd.y > 2. && abs(pd.x) > 10. && pd.y < 5.) {
        float fl = floor(pd.z / 13.); lcol = 0.4 * lcol + 0.5 * vec3(hash(.1562 + fl), hash(.423134 + fl), 0.);
    }
    if(abs(pd.x) > 10. && pd.y > 5.) {
        float fl = floor(pd.z / 2.); lcol = 0.5 * lcol + 0.5 * vec3(hash(.1562 + fl), hash(.923134 + fl), hash(.423134 + fl));
    }
    return lcol;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {    
    ...
    if (q.y < .12 || q.y >= .88) { fragColor = vec4(0., 0., 0., 1.); return; }
    ...
    if(ints > -0.5) {
        ...
        if(abs(int1.x) < 8.) {
            if(int1.y < 0.01) r = 0.007 * fbm(int1.xz);
            else r = 0.02;
        }
        if(abs(int1.x) < 0.1 || abs(abs(int1.x) - 7.4) < 0.1) r *= 4.;
        r *= 2.;
        col = shade(ro, int1.xyz, nor1);
        if(ints > 0.5) col += r * shade(int1.xyz, int2.xyz, calcNormalSimple(int2.xyz));
        if(lint2.w > 0.) col += (r * LIGHTINTENSITY * exp(-lint2.w * 7.0)) * getLightColor(lint2.xyz);
    } 
    ...
    if(lint1.w > 0.) col += (f * LIGHTINTENSITY * exp(-lint1.w * 7.0)) * getLightColor(lint1.xyz);
...
const HeavyRainShader: React.FC = () => {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-[#050505]">
      ...
      />
      <div className="absolute inset-0 z-10 bg-[radial-gradient(ellipse_at_center,transparent_0%,#000_100%)]" />
    </div>
  );
};

export default HeavyRainShader;
